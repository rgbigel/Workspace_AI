...
> *Exported on: 2026-08-26 22:55:20 | Host: D5P0-SSD980-Z | Version: 1.2.0*
...
4. **Applies Universally To**:
   ...
   - GUI Windows & WPF Launchers (`Show-LcmActiveLauncher.ps1`)
...
MACRO: @tools
- description: display authoritative alphabetical index of LCM and HaSSD06 tools
- aliases: @t, @tools, @menu
- rules:
  - '@tools' or '@t' -> executes 'pwsh -File tools/Show-ToolIndex.ps1 -Audience User'
  - '@tools dev' or '@t dev' -> executes 'pwsh -File tools/Show-ToolIndex.ps1 -Audience Dev'
  - '@tools ha' or '@t ha' -> executes 'pwsh -File tools/Show-ToolIndex.ps1 -Group HaSSD06'
  - '@tools lcm' or '@t lcm' -> executes 'pwsh -File tools/Show-ToolIndex.ps1 -Group LCM'
  - '@tools all' or '@t all' -> executes 'pwsh -File tools/Show-ToolIndex.ps1 -Audience All'
  - supports -Filter <query> and -Grid
...
|:---|:---|:---|:---|:---|
| **[ProposalReviewFlowPolicy.md](file:///.agents/rules/ProposalReviewFlowPolicy.md)** | `RULE-LCM-001` - `006` | **Proposal & Review Flow** | Workspace & Child Repos | Proposal-first intent, batch commands (`do`, `delete`, `defer`), Beyond Compare 5 review gate, dual-commit sync. |
| **[PowerShellStandardsPolicy.md](file:///.agents/rules/PowerShellStandardsPolicy.md)** | `RULE-PS-001` - `011` | **PowerShell Standards** | All `*.ps1`, `*.psm1`, `*.psd1` | StrictMode `@(...)` wrapping, Microsoft approved verbs (`Get-Verb`), colon-safe string interpolation, test elevation gating, header metadata & date maintenance, structured logging, `-h` help, and interactive desktop dispatch routing. |
| **[ReviewCommitGovernancePolicy.md](file:///.agents/rules/ReviewCommitGovernancePolicy.md)** | `RULE-REV-001` - `005` | **Commit Gating & RR** | Governed Repos & Root | Mandatory review-gated commits (`ACCEPTED`), readiness quality gate pass, audit receipts in `Workspace_Inventory/data/reviews/`. |
...
| **[ElevationPolicy.md](file:///.agents/rules/ElevationPolicy.md)** | `ELEVATION-POLICY` | **Security & Privileges** | Workspace-wide | Least-privilege execution default, elevated script runner delegation, auto-detection of privileged commands. |
...
| **[DocumentationStandardsPolicy.md](file:///.agents/rules/DocumentationStandardsPolicy.md)** | `RULE-DOC-001` - `004` | **Documentation Standards** | All `*.md`, `docs/`, `install/` | Tripartite specifications (`Architecture.md`, `Requirements.md`, `Implementation.md`), universal `install/Installation.md` runbook, DOX metadata headers. |
