# Workspace_AI Real-Repository Dry-Run Cases
...
- 2026-08-02: Clarified Workspace_AI-only incubation, candidate selection, override, rollback, and Accept-gate policy.
...
This document describes the current read-only dry-run model for applying Workspace_AI governance against a future real repository. It is intentionally not a write workflow. It documents the cases that are currently implemented and validated by the native readiness command.

## Current State

Workspace_AI is still the methodology incubation repository. Real-repository testing starts by selecting a repository that already exists under `D:\Git_Repositories`, but methodology evolution itself remains in Workspace_AI until further notice.
...
dry_run.enabled: false in Workspace_AI
...
Nothing is copied to `D:\Git_Repositories` as a parent-level rule location. The real workspace is the repository set under `D:\Git_Repositories`; Workspace_AI remains the place where methodology changes are designed, validated, and accepted before any real repository is changed.

Workspace_AI may record the candidate identity, but it must not own the target repository's dry-run results, work logs, proposals, or repo-specific decisions. Those belong inside the target repository once its target-local method instance exists.

## Safety Rules

- Workspace_AC is off-limits.
- `B:\Backups\Base_WS_AC` is off-limits.
- Workspace_AI cannot select itself as a target.
- Selecting a repository does not enable writes.
- Selecting a repository records only the candidate in Workspace_AI.
...
Repo-local rules can override Workspace_AI only when they are confirmed as repository requirements, not merely incidental implementation details. That decision must be documented in the target repository before it is treated as an override.
...
Workspace_AI defines the method baseline. The target repository owns the application of that method.
...
Workspace_AI may hold generic rules, templates, validation logic, and the candidate identity. It must not store target-repo dry-run results, work logs, repo-specific proposals, or repo-specific verification results.
...
The bootstrap is allowed only after explicit operator approval. It may create the target-local method directories, a manifest, and local README files inside the target repository. It must not stage, commit, or write unrelated target files, and it must not move target-repo outputs back into Workspace_AI.

The old idea of enabling a real-repo dry-run by writing more state into Workspace_AI is transitional only. Future target work should first establish the target-local `Docs/Methods` method instance, then store dry-run state and results there.

## Repository Lifecycle Boundary Cases

Being located under `D:\Git_Repositories` is not enough to make a repository eligible for Workspace_AI operations. A repository can be discovered but blocked, selected as a candidate, placed in read-only dry-run, treated as external intake, considered for cleanup, governed after explicit acceptance, or retired.
...
method baseline proposals: .copilot/History/Logs/Proposals.json
```
...
Workspace_AI does not store change proposals for ordinary target repos. The exception is when Workspace_AI itself is the method-baseline target; in that case the existing `tools/...` proposal mechanism is appropriate.
...
Workspace_AI must not perform hidden integrity scans, periodic background checks, or repo-wide checksums by default.
...
Escalated checks are allowed only after a trigger or ambiguous cheap result, and Workspace_AI must report before using them:
...
Macrium or system restore events are out-of-band. Workspace_AI does not plan them. If their aftermath is detected, normal operations stop and the method reports a diagnostic or repair path; it does not auto-repair.

## Case 1: No Repository Selected

Purpose: prove that Workspace_AI can validate its dry-run machinery without inspecting any external repository.
...
Selection is rejected if the path is Workspace_AI, Workspace_AC, the backup base, or not an existing Git repository.
...
Dry-run activation still requires explicit read-only confirmation, but the dry-run's repo-specific state and results must be target-local. Workspace_AI must not become the storage location for those target work products.
...
- presence or absence of expected Workspace_AI adapter surfaces.
...
Purpose: show what Workspace_AI would intend to do in a later write-capable adaptation pass, without performing those actions now.
...
Each preview action compares the Workspace_AI source surface with the target repository surface using read-only SHA256 hashes. Comparison states are:
...
```text
phase-01-structure                  -> check repository structure against Workspace_AI workspace rules
